    ...
    <QueryClientProvider client={queryClient}>
      <MemoryRouter initialEntries={['/login']}>
        <Routes>
          ...
          <Route path="/dashboard/profile" element={<div>Artist dashboard</div>} />
...
describe('LoginForm', () => {
  ...
  it('creates a pending artist profile after artist login', async () => {
    const user = userEvent.setup()
    setPendingArtistProfile({ artistic_name: 'La Montaña', city: 'Pasto' })
    mockedLogin.mockResolvedValueOnce({
      tokens: { access: 'access-token', refresh: 'refresh-token' },
      user: {
        id: 2,
    ...
    expect(localStorage.getItem('narino_cultura_pending_artist_profile')).toBeNull()
  })
})
