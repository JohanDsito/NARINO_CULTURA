...
describe('RegisterForm', () => {
  ...
  it('requires artist-specific fields when role is artist', async () => {
    ...
    await user.type(screen.getByLabelText(/email/i), 'artist@test.com')
    await user.type(screen.getByLabelText(/contraseña/i), 'secret123')
    await user.selectOptions(screen.getByLabelText(/tipo de usuario/i), 'artist')
    await user.click(screen.getByRole('button', { name: /crear cuenta/i }))

    expect(await screen.findByText(/nombre artístico es obligatorio/i)).toBeInTheDocument()
  ...
  it('maps buyer registration to backend role and redirects to login', async () => {
    const user = userEvent.setup()
    mockedRegister.mockResolvedValueOnce({ message: 'Registro exitoso.' })
    renderRegisterForm()
